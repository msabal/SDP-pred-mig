...
F.vec[(Xcrit+1):Xmax,2,10] <- T.FITNESS(X = seq((Xcrit+1), Xmax ,1), m=5)  #if salmon make it to location 10, then their fitness depends on their energy reserves.
F.vec[0:Xcrit,2,10] <- 0
...
#  OVER.BEH(B, X=2, Xmax=10, L=10, Lmax=10, e, e10, c, b, F.vec) # see output of OVER.BEH function. ERROR HERE!?
#  OVER.BEH(B, X=1, Xmax=10, L=1, Lmax=10, e, e10, c, b, F.vec) # Common to have problems when X=1...use this to check. ERROR HERE?!
...
df.all1 <- join(df.beh, df.W)   # join data for best behavior and fitness together in one dataframe.
df.all1$Best.beh <- as.factor(df.all1$Best.beh)

df.all1 <- df.all1[df.all1$Time != 30,]  # drop Time = 30 because that is the end, and therefore have no behavioral choices.
...
ggplot(data=df.all1, aes(x=Time, y=X, fill=Best.beh)) + geom_tile(color="gray90") + facet_wrap(~ L, ncol = 2) +
...
ggplot(data=df.all1, aes(x=Time, y=X, fill=W)) + geom_tile() + facet_wrap(~ L, ncol = 5) + coord_equal() +
  theme(axis.line = element_line(colour = "black"), panel.border = element_blank(), panel.background = element_blank())
