...
for(i in 1:length(yn)) {
  ...
  OUT$param_value <- rep(yn[i], length(OUT$Wstart)) # add column with seeds value for that iteration.
  OUT$param_name <- rep("yn", length(OUT$Wstart))
...
for(i in 1:length(ya)) {
  ...
  OUT$param_value <- rep(ya[i], length(OUT$Wstart)) # add column with seeds value for that iteration.
...
for(i in 1:length(Ba)) {
  ...
  OUT$param_value <- rep(Ba[i], length(OUT$Wstart)) # add column with seeds value for that iteration.
...
for(i in 1:length(d)) {
  ...
  OUT <- MAIN_FUN(Wc, A, t, U, Wmax, Wmin, Amax, # state vars, constraints & beh choice (vars we will for loop over)
                  E, q, a, Alpha, d[i], v, f, g, c, j, Bu, Bw, M, m, y, P, z, # vars in functions
                  ya, yn, yo, dn0, Ba, Bn, Bo, ka, kn, # vars that vary by habitat (h.vec)
                  Ws, r, Smax, W, # vars for Terminal fitness function
                  Wstep.n, Wstep, tmax, seeds, F.vec, N)
  ...
  OUT$param_value <- rep(d[i], length(OUT$Wstart)) # add column with seeds value for that iteration.
...
# Export csv file!!!!
write.csv(DF.ALL, "iterate_sum_null_and_hab.csv")
