#### To run on the OSU servers ####

# Null habitat hypotheses: (1) predator abundance, (2) escape ability, 
# (3) foraging gain, (4) energy refugia.

# Habitat hypotheses: repeat

# Includes:
# Baseline parameters - habitat-dependent
# All functions
# iterate summary - yn, ya, Bn, Ba, kn, ka, dn0, d

# Baseline parameters - null habitat
# iterate summary - yn, ya, Bn, Ba, kn, ka, dn0, d

# Bind all data frames together: save file
...
## Iterate Main Function over: Bn when yn > ya ----

# When there are more predators in natural vs. altered:
ya <- 0.7 # 30% less
yn <- 1

# When there is 25% natural habitat only
N <- 0.75 # N defines how much altered habitat there is.
...
for(i in 1:length(ka)) {
  ...
  OUT$param_value <- rep(ka[i], length(OUT$Wstart)) # add column with seeds value for that iteration.
...
ka <- param_dat['null','ka'] # put var back to baseline: null
...
# Export csv file!!!!
#write.csv(DF.ALL, "C://Users//sabalm//Desktop//iterate_when_yn_greater_ya_alone_25natural.csv")

write.csv(DF.SUM, "C://Users//sabalm//Desktop//iterate_when_yn_greater_ya_25natural.csv")

# Later in post-processing script, compare p.tot for Beh == 0 by interation between altered vs. natural (are there more pauses in natural vs. altered.)
# d <- aggregate(p.tot ~ Beh + h + param_value, DF.SUM, mean)
# subset(d, Beh == 0)
# This practice example shows that YES, they can still choose to pause at natural even if there are 30% more predators there. Awesome!